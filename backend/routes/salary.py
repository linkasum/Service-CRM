...
def create_salary_records(
    ...
    # Берём ВСЕ транзакции за период (не только issued)
    all_txs = session.exec(
        select(CashTransaction).where(
            CashTransaction.order_id.isnot(None),
            CashTransaction.created_at >= date_from,
            CashTransaction.created_at <= date_to_end,
        )
    ).all()
    
    # Группируем по master_id через order
    master_net: dict = {}  # master_id -> {order_ids: set, cash_net, card_net, total_cost, parts_cost}
    for tx in all_txs:
        order = session.get(Order, tx.order_id)
        if not order or not order.master_id:
            continue
        mid = order.master_id
        if mid not in master_net:
            master_net[mid] = {"order_ids": set(), "cash_net": 0.0, "card_net": 0.0, "total_cost": 0.0, "parts_cost": 0.0}
        master_net[mid]["order_ids"].add(tx.order_id)
        amt = tx.amount or 0
        if (tx.payment_method or '').lower() == 'card':
            master_net[mid]["card_net"] += amt
        else:
            master_net[mid]["cash_net"] += amt
    
    # Добираем total_cost и parts_cost для заказов
    for mid in master_net:
        oids = list(master_net[mid]["order_ids"])
        if oids:
            orders_data = session.exec(
                select(Order).where(Order.id.in_(oids))
            ).all()
            master_net[mid]["total_cost"] = sum(o.total_cost or 0 for o in orders_data)
            master_net[mid]["parts_cost"] = sum(o.parts_cost or 0 for o in orders_data)
    
    created = 0
    for master_id, data in master_net.items():
        total_cost = data["total_cost"]
        parts_cost = data["parts_cost"]
        cash_net = data["cash_net"]
        card_net = data["card_net"]
        ...
        if config:
            if config.config_type == SalaryType.fixed:
                ...
                formula = formula.replace("total", str(total_cost))
                formula = formula.replace("parts", str(parts_cost))
                formula = formula.replace("{orders_count}", str(orders_count))
...
def salary_report(
    ...
    if date_from:
        count_query = count_query.where(SalaryRecord.period_start >= date_from)
    if date_to:
